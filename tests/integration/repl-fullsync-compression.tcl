...
#
# repl-compress-sync is MODIFIABLE and applies to subsequent syncs, so each test
# sets the value it needs at its start (lz4 or no) and repopulates its own key
...
start_server {tags {"repl rdb-compression external:skip needs:debug"} overrides {save "" enable-debug-command local}} {
    ...
    $primary config set repl-compress-sync lz4
    $primary config set rdb-del-sync-files no

    # --- case 1: single capable replica -> COMPRESSED + correct -----------
    start_server {overrides {save "" enable-debug-command local repl-compress-sync lz4}} {
    ...
    # compress-sync capability follows its own repl-compress-sync, set per
    # iteration; the primary's mode is set per iteration too.
    start_server {overrides {save "" enable-debug-command local repl-compress-sync lz4}} {
        set replica [srv 0 client]

        foreach {scenario primary_mode replica_mode prefix} {
            primary-compression-off no  lz4 off
            replica-not-capable     lz4 no  nocap
        } {
            test "Disk full sync: $scenario yields a plaintext RDB that loads" {
                $primary config set repl-compress-sync $primary_mode
                $replica config set repl-compress-sync $replica_mode
    ...
    start_server {overrides {save "" enable-debug-command local repl-compress-sync lz4 repl-diskless-load swapdb}} {
        set replica [srv 0 client]
...
start_server {tags {"repl rdb-compression external:skip needs:debug"} overrides {save "" enable-debug-command local}} {
    ...
    start_server {overrides {save "" enable-debug-command local repl-compress-sync lz4}} {
        ...
        start_server {overrides {save "" enable-debug-command local repl-compress-sync no}} {
...
#
# Each test sets its own repl-compress-sync via CONFIG SET, repopulates its own
    ...
    start_server {overrides {save "" enable-debug-command local repl-compress-sync lz4}} {
        ...
        start_server {overrides {save "" enable-debug-command local repl-compress-sync no}} {
            ...
            test {Plain disk BGSAVE in flight is joinable by a non-capable replica} {
                $primary config set repl-compress-sync no
...
# Requires `repl-compress-sync lz4` on both primary and replica.
# ============================================================================
...
#
# dual-channel-replication-enabled yes, repl-diskless-sync yes, repl-compress-sync
...
start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-sync yes repl-diskless-sync-delay 0 dual-channel-replication-enabled yes}} {
    set primary [srv 0 client]
    set primary_host [srv 0 host]
    set primary_port [srv 0 port]
    ...
    foreach load_mode {disabled swapdb} {
        test "Dual-channel + compression full sync delivers identical data (repl-diskless-load $load_mode)" {
            ...
            start_server [list overrides [list save "" repl-compress-sync lz4 repl-diskless-load $load_mode dual-channel-replication-enabled yes]] {
                set replica [srv 0 client]
                set replica_loglines [count_log_lines 0]
                $replica replicaof $primary_host $primary_port
    ...
    test {Dual-channel diskless full sync counts output bytes across all sockets} {
        ...
        start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-load swapdb dual-channel-replication-enabled yes}} {
...
#
# repl-diskless-sync yes, repl-diskless-sync-delay 0, repl-compress-sync lz4,
...
start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-sync yes repl-diskless-sync-delay 0}} {
    set primary [srv 0 client]
    set primary_host [srv 0 host]
    set primary_port [srv 0 port]
    ...
    test {Disk-receive (repl-diskless-load disabled) + diskless compressed save loads correctly} {
        ...
        start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-load disabled}} {
            set replica [srv 0 client]
            set replica_loglines [count_log_lines 0]
            $replica replicaof $primary_host $primary_port
    ...
    #    repl-compress-sync lz4 AND the attaching replica advertised the
    ...
    test {Diskless full sync compresses the RDB payload for a capable replica} {
        ...
        start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-load swapdb}} {
            set replica [srv 0 client]
            set replica_loglines [count_log_lines 0]
            $replica replicaof $primary_host $primary_port
...
start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-sync yes repl-diskless-sync-delay 0 rdbchecksum no}} {
    set primary [srv 0 client]
    set primary_host [srv 0 host]
    set primary_port [srv 0 port]
...
start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-sync yes repl-diskless-sync-delay 0}} {
    ...
    start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-load swapdb}} {
        ...
        start_server {overrides {save "" repl-compress-sync no repl-diskless-load swapdb}} {
            set replica_plain [srv 0 client]
            ...
            test {Mixed diskless cohort falls back to a plaintext RDB payload} {
                ...
                # AND-rule result: the round is plaintext (no compression NOTICE),
                # and nobody is skipped so no mismatch NOTICE fires.
...
start_server {overrides {save "" repl-compress-sync lz4 repl-diskless-load swapdb}} {
    ...
    #     succeeds and the finish-path credit must count every encoded wire byte:
    #     total_net_repl_input_bytes == payload bytes N + the "$<N>\r\n" bulk
    #     header. Tight enough to catch a lost footer unlike the comp < plain/2
    #     check.
    test {Compressed socket load credits exactly the encoded wire bytes on a successful load} {
        assert_equal "VCS" [string range $fsc_payload 0 2]
    ...
    #     flushed on the failing path. Truncation is recoverable (not the
    #     corruption exit), so the replica survives and total_net_repl_input_bytes
    #     must equal the bytes actually sent plus the "$<N>\r\n" bulk header.
    test {Compressed socket load credits every wire byte received on a failing (truncated) transfer} {
        assert_equal "VCS" [string range $fsc_payload 0 2]
