...
proc rdb_is_compressed {client} {
    set header [read_binary_file_prefix [dump_rdb_path $client] 3]
...
# row via CONFIG SET. rdb-key-save-delay 13000 (~2s over 155 keys) keeps the
# save in flight while the joiner arrives.
# ============================================================================
    ...
    start_server {overrides {save "" rdbcompression lz4 repl-diskless-load swapdb}} {
        ...
        start_server {overrides {save "" rdbcompression yes repl-diskless-load swapdb}} {
            ...
            test {Mixed diskless cohort falls back to a plaintext RDB payload} {
                ...
                assert_replica_synced $primary $replica_capable \
                    "Capable replica digest mismatch after mixed diskless full sync"
                assert_replica_synced $primary $replica_plain \
...
start_server {overrides {save "" rdbcompression lz4}} {
    ...
    $src save
    set src_dump [dump_rdb_path $src]
