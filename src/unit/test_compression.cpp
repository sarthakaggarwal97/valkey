...
TEST_F(CompressionTest, streamReaderGetsChecksumInfoWithoutAnotherSourceRead) {
    ...
    for (bool codec_checksum : {false, true}) {
        ...
        ASSERT_TRUE(info.compressed);
        ASSERT_FALSE(info.codec_checksum_info_available);
        ASSERT_EQ(source.calls, 2) << "classification should read only the 3-byte magic and 4-byte envelope tail";

        char out[64] = {0};
        ASSERT_EQ(streamReaderRead(&reader, out, strlen(payload)), (ssize_t)strlen(payload));
        ASSERT_EQ(memcmp(out, payload, strlen(payload)), 0);
        int calls_after_decode = source.calls;

        ASSERT_EQ(streamReaderGetInfo(&reader, &info), 0);
        ASSERT_TRUE(info.codec_checksum_info_available);
        ASSERT_EQ(info.codec_block_checksum_enabled, codec_checksum);
        ASSERT_EQ(info.codec_content_checksum_enabled, codec_checksum);
        ASSERT_EQ(source.calls, calls_after_decode)
            << "frame metadata must come from the active decoder, not another read";
