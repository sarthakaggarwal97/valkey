...
static streamPushReaderResult pushReaderFeedInput(streamPushReader *reader, const void *src, size_t len, size_t *input_consumed, sds *out, size_t output_budget) {
    ...
    *input_consumed = 0;
    ...
    if (reader->state == STREAM_PUSH_READER_PROBE) {
        while (reader->envelope_len < VCS_MAGIC_SIZE && off < len) {
            reader->envelope[reader->envelope_len++] = in[off++];
            if (!vcsHasMagicPrefix(reader->envelope, reader->envelope_len)) {
                reader->state = STREAM_PUSH_READER_PASSTHROUGH;
                break;
            }
        }
        ...
        if (reader->state == STREAM_PUSH_READER_PROBE) {
            ...
            while (reader->envelope_len < VCS_ENVELOPE_SIZE && off < len)
                reader->envelope[reader->envelope_len++] = in[off++];
            if (reader->envelope_len < VCS_ENVELOPE_SIZE) {
                *input_consumed = off;
            ...
            if (streamDecompressorInit(&reader->decompressor, algo, false) != C_OK) {
                *input_consumed = off;
                return STREAM_PUSH_READER_ERR;
            }
            reader->state = STREAM_PUSH_READER_COMPRESSED;
        }
    }

    if (reader->state == STREAM_PUSH_READER_PASSTHROUGH) {
        /* Replay any buffered magic-prefix bytes once, then forward the rest. */
        *out = sdscatlen(*out, reader->envelope, reader->envelope_len);
        reader->envelope_len = 0;
...
streamPushReaderResult streamPushReaderFeed(streamPushReader *reader, const void *src, size_t len, sds *out, size_t output_budget) {
    ...
    if (reader->pending_input) {
        ...
        if (reader->pending_input_pos == sdslen(reader->pending_input)) {
            sdsfree(reader->pending_input);
            reader->pending_input = NULL;
            reader->pending_input_pos = 0;
        }
