...
int streamReadEnvelopeInfo(const uint8_t *buf,
    ...
    info->codec_checksum_info_available = false;
    info->codec_block_checksum_enabled = false;
    info->codec_content_checksum_enabled = false;
    info->algo = algo;
...
int streamReaderGetInfo(streamReader *reader, streamReaderInfo *info) {
    ...
    if (reader->probe.compressed) {
        int frame_info_rc = streamDecompressorGetFrameInfo(&reader->decompressor);
        if (frame_info_rc < 0) return -1;
        if (frame_info_rc == 0) {
            info->codec_checksum_info_available = true;
            info->codec_block_checksum_enabled = reader->decompressor.block_checksum_enabled;
            info->codec_content_checksum_enabled = reader->decompressor.content_checksum_enabled;
        }
    }
    return 0;
}
