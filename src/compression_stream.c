...
int vcsWriteEnvelope(uint8_t *buf, compressionAlgo algo, uint8_t stream_kind) {
    uint8_t codec;
    switch (algo) {
    case ALGO_LZ4:
        codec = VCS_CODEC_LZ4;
...
static int writeVcsEnvelope(streamWriterWriteFn write_cb,
    ...
    uint8_t envelope[VCS_ENVELOPE_SIZE];
    if (vcsWriteEnvelope(envelope, algo, VCS_STREAM_RDB) == C_ERR) return C_ERR;
...
int vcsReadEnvelope(const uint8_t *buf, uint8_t expected_stream_kind, compressionAlgo *algo) {
    if (!vcsHasMagicPrefix(buf, VCS_MAGIC_SIZE)) return C_ERR;
...
/* Emit the envelope lazily so an unused writer produces no output. */
...
int streamReaderInit(streamReader *reader, const streamReaderConfig *cfg, streamReaderReadFn read_cb, void *read_ctx, compressionAlgo *detected_algo) {
    ...
    while (true) {
        ...
        if (reader->probe.header_len == VCS_ENVELOPE_SIZE) {
            if (vcsReadEnvelope(reader->probe.header, VCS_STREAM_RDB, &algo) == C_ERR) {
                streamReaderSetError(reader, STREAM_READER_ERROR_INCOMPATIBLE);
                return C_ERR;
            }
