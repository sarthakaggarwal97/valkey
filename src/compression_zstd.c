...
size_t compressionZstdOutputBound(size_t input_len) {
     ...
     * contiguous input. Add headroom for the frame header (max 18 bytes),
     * optional content checksum (4 bytes), and flush/end overhead so the
     * caller can allocate once and reuse for any flush mode. */
    return ZSTD_compressBound(input_len) + 22;
}
...
ssize_t compressionZstdCompressFeed(stream_compressor_t *sc,
    ...
    /* For FLUSH_SYNC and FLUSH_END, ret > 0 means "not yet fully flushed,
     * call again". Loop until the directive is satisfied. */
    while (ret > 0 && (flush_mode == FLUSH_SYNC || flush_mode == FLUSH_END)) {
        /* All input was already provided; pass empty input on subsequent calls. */
        in_buf.src = NULL;
        in_buf.size = 0;
        in_buf.pos = 0;
        ret = ZSTD_compressStream2(cctx, &out_buf, &in_buf, zstd_directive);
