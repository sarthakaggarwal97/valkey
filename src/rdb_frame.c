...
ssize_t rdbFrameFormatConfigLine(char *buf, size_t buf_len, int codec, size_t block_bytes) {
    ...
    if (codec_str == NULL) return -1;

    int len = snprintf(buf, buf_len, "codec=%s blk=%zu", codec_str, block_bytes);
    if (len < 0) return -1;
...
rdbFrameParseResult rdbFrameParseConfigLine(char *buf,
    ...
    if (buf == NULL || codec_token == NULL || blk_token == NULL) {
    ...
    *blk_token = NULL;
    ...
    if (field_count != 2 || *codec_token == NULL || *blk_token == NULL) {
        return RDB_FRAME_PARSE_INVALID_FORMAT;
    }
